...
import { GetAdminProfit } from '../../services/AdminWallet';
...
const AdminProfit = () => {
  ...
  const BindAdminProfit = async ({ dateRange, selectedApi }) => {
    try {      
      var requestdata = encryptvalue(JSON.stringify({
      ...
      const _result = await GetAdminProfit({ data: requestdata });    
      setAdminProfit(_result);
    } catch (err) {
      Swal.fire("warning!", err.message, "warning");
    } finally {
  ...
  const handleApiChange = (e) => {
    ...
    BindAdminProfit({ dateRange: dateRange, selectedApi: apiId });
  }
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'Collection Surcharge', 'Deduct Surcharge', 'Profit', 'Downline Commission'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  ) : adminProfit && Array.isArray(adminProfit.data) && adminProfit.data.length > 0 ? (
                    adminProfit.data.map((row, index) => (
                      <tr key={index + 1}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{index + 1}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.TotalSurchargeCollection)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.AdminDeductSurCharge)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.AdminProfit)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.TotalCommissionDistribute)}</td>
                      </tr>
                    ))
                  ) : (
                    <tr key={0}>
