...
import { GetManualCommissionLedger } from '../../services/AdminWallet';
...
const ManualCommCrDrLedger = () => {
  ...
  const [ManualCommissionLedger, setManualCommissionLedger] = useState();
  ...
  const fetchInitialData = async () => {
    try {
      ...
      var apirequestData = encryptvalue(JSON.stringify({
        serviceName: "Payin"
      }));
      const _apiresult = await BindAPIListByServiceName({ data: apirequestData });
  ...
  useEffect(() => {
    (async () => {
      try {
        BindManualCommissionLedger({ selectedUserId: selectedUserId, selectedApi: selectedApi, dateRange: dateRange, currentPage: currentPage });
      } catch (err) {
        Swal.fire("warning!", err.message, "warning");
      } finally {
  ...
  const BindManualCommissionLedger = async ({ selectedUserId, selectedApi, dateRange, currentPage }) => {
    try {
      var requestData = encryptvalue(JSON.stringify({
      ...
      const _result = await GetManualCommissionLedger({ data: requestData });
      if (_result && Array.isArray(_result.data) && _result.data.length > 0) {
        setManualCommissionLedger(_result);
        setTotalCount(_result.data[0].TotalRecord)
      } else {
        setManualCommissionLedger([]);
        setTotalCount(0);
      }
    } catch (err) {
      Swal.fire("warning!", err.message, "warning");
  ...
  const BindDataUserChange = (e) => {
    ...
    BindManualCommissionLedger({ selectedUserId: userId, selectedApi: selectedApi, dateRange: dateRange, currentPage: currentPage });
  }

  const handleApiChange = (e) => {
    const apiId = parseInt(e.target.value);
    setSelectedApi(apiId);
    BindManualCommissionLedger({ selectedUserId: selectedUserId, selectedApi: apiId, dateRange: dateRange, currentPage: currentPage });
  }
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Manual Commission Ledger</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              ...
              onClick={() => BindManualCommissionLedger({ selectedUserId, selectedApi, dateRange, currentPage })}
            >
              Search
            </button>
          </div>
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'API Name', 'Transaction Type', 'Amount', 'Date & Time', 'Remarks'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  ) : ManualCommissionLedger && Array.isArray(ManualCommissionLedger.data) && ManualCommissionLedger.data.length > 0 ? (
                    ManualCommissionLedger.data.map((row, index) => (
                      <tr key={index + 1}>
