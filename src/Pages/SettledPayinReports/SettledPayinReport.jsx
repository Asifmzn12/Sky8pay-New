...
const SettledPayinReport = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        BindSettledPayinReport({ dateRange: dateRange, selectedUser: selectedUser, selectedApi: selectedApi, searchValue: "", currentPage: currentPage });
      }
      catch (err) {
        Swal.fire("warning!", err.message, "warning");
      } finally {
  ...
  const BindSettledPayinReport = async ({ dateRange: dateRange, selectedUser: selectedUser, selectedApi: selectedApi, searchValue: searchValue, currentPage: currentPage }) => {
    ...
    try {
      const _result = await GetsettledPayinReport({
        ...
        apiId: selectedApi, status: StatusEnum.Success, search: searchValue, transactionTypeId: 0,
  ...
  const BindDataUserChange = (e) => {
    ...
    BindSettledPayinReport({ dateRange: dateRange, selectedUser: userId, selectedApi: selectedApi, searchValue: "", currentPage: currentPage });
  }
  ...
  const BindDataApiChange = (e) => {
    ...
    BindSettledPayinReport({ dateRange: dateRange, selectedUser: selectedUser, selectedApi: apiId, searchValue: "", currentPage: currentPage });
  }
  return (
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Settled Payin Report</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              onClick={() => BindSettledPayinReport({ dateRange, selectedUser, selectedApi, searchValue, currentPage })}
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'Service Name', 'Date & Time', 'Status', 'Amount', 'RRN', 'Name', 'Account No', 'IFSC', 'UPI', 'Open', 'Surcharge', 'GST', 'Payable', 'Closed', 'Ref Id', 'Unique Id', 'Receipt', 'Complaint', 'Chargeback', 'Txn From', 'CallBack', 'Check Status', 'Fraud', 'IP Address', 'Invoice', 'IP State', 'IP City'].map(header => (
                  ...
                  {loadingTable ? (
                    ...
                    PayinReportList.data.map((row, index) => (
                      <tr key={row.Id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.RasieChargeBack}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <BsDownload size={20} color='black' onClick={() => DownloadPayinInvoice(row.SystemUniqueId)} />
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.IPCity?row.IPCity:"NA"}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.IPState?row.IPState:"NA"}</td>
                      </tr>
                    ))
                  ) : (
                    <tr key={0}>
...
export default SettledPayinReport