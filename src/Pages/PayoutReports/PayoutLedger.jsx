...
const PayoutLedger = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        BindPayoutLedger({ dateRange: dateRange, selectedApi: selectedApi, selectedUser: selectedUser, currentPage: currentPage, selectedStatus: selectedStatus, searchValue: "" });
      }
      catch (err) {
        Swal.fire("warning!", err.message, "warning");
      } finally {
  ...
  const BindPayoutLedger = async ({ dateRange, selectedApi, selectedUser, currentPage, selectedStatus, searchValue }) => {
    try {
      const _result = await GetPayoutLedger({
        ...
        endDate: dateRange?.[1]?.format("YYYY-MM-DD"), apiId: selectedApi, status: selectedStatus,
        search: searchValue, pageNo: currentPage, pageSize: rowsPerPage
      });
  ...
  const BindDataUserChange = (e) => {
    ...
    BindPayoutLedger({ dateRange: dateRange, selectedApi: selectedApi, selectedUser: userId, currentPage: currentPage, selectedStatus: selectedStatus, searchValue: "" });
  }
  ...
  const BindDataApiChange = (e) => {
    ...
    BindPayoutLedger({ dateRange: dateRange, selectedApi: apiId, selectedUser: selectedUser, currentPage: currentPage, selectedStatus: selectedStatus, searchValue: "" });
  }
  const BindDataStatusChange = (e) => {
    const statusId = parseInt(e.target.value);
    setSelectedStatus(statusId);
    BindPayoutLedger({ dateRange: dateRange, selectedApi: selectedApi, selectedUser: selectedUser, currentPage: currentPage, selectedStatus: statusId, searchValue: "" });
  }
  return (
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              onClick={() => BindPayoutLedger({ dateRange, selectedApi, selectedUser,  currentPage,  selectedStatus, searchValue })}
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'Service Name', 'Date & Time', 'Status', 'Amount', 'RRN', 'Type', 'Open', 'Commission', 'TDS', 'Surcharge', 'GST', 'Payable', 'Closed', 'Unique Id', 'Comment'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {loadingTable ? (
                    ...
                    PayoutLedgerList.data.map((row, index) => (
                      <tr key={row.Id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.ApiName ? row.ApiName : row.ServiceName}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.BankUTRNo}</td>
