  ...
  const BindPayoutReport = async ({ dateRange, selectedUser, selectedApi, searchValue, currentPage }) => {
    try {
      const _result = await GetPayoutReports({
        ...
        endDate: dateRange?.[1]?.format("YYYY-MM-DD"), apiId: selectedApi, status: StatusEnum.Failed, search: searchValue,
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'Service Name', 'Date & Time', 'Mode', 'Status', 'RRN', 'Amount', 'Name', 'Account No', 'IFSC', 'UPI', 'Open', 'Surcharge', 'GST', 'Payable', 'Closed', 'Ref Id', 'Unique Id', 'API Ref Id', 'Receipt', 'Complaint', 'Txn From', 'CallBack', 'Check Status', 'IP Address'].map(header => (
                  ...
                  {loadingTable ? (
                    ...
                    PayoutReportList.data.map((row, index) => (
                      <tr key={row.Id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white max-w-[140px] truncate" title={row.AccountHolderName}>{row.IpAddress}</td>
                      </tr>
                    ))
                  ) : (
                    <tr key={0}>
...
export default FailedPayoutReport