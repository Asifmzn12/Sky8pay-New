...
import { CheckStatusTransaction, GetPayoutReports } from '../../services/PayoutReport';
...
import { BsEye, BsReceipt } from 'react-icons/bs';
...
const PendingPayoutReport = () => {
  ...
  const BindPayoutReport = async ({ dateRange, selectedUser, selectedApi, searchValue, currentPage }) => {
    try {
      const _result = await GetPayoutReports({
        ...
        endDate: dateRange?.[1]?.format("YYYY-MM-DD"), apiId: selectedApi, status: StatusEnum.Pending, search: searchValue,
  ...
  const CallCheckStatusTransaction = async (payoutId = 0) => {
    try {
      const _result = await CheckStatusTransaction({ PayoutId: payoutId });
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Pending Payout Report</h1>
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'Service Name', 'Date & Time', 'Mode', 'Status', 'RRN', 'Amount', 'Name', 'Account No', 'IFSC', 'UPI', 'Open', 'Surcharge', 'GST', 'Payable', 'Closed', 'Ref Id', 'Unique Id', 'API Ref Id', 'Receipt', 'Complaint', 'Txn From', 'Check Status', 'IP Address'].map(header => (
                  ...
                  {loadingTable ? (
                    ...
                    PayoutReportList.data.map((row, index) => (
                      <tr key={row.Id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium"><BsEye size={20} color='black' onClick={() => CallCheckStatusTransaction(row.Id)} /></td>
...
export default PendingPayoutReport