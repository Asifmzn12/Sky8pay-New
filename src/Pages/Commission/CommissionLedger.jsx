...
import { GetCommissionLedger } from '../../services/Commission';
...
const CommissionLedger = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        BindCommissionLedger({ dateRange: dateRange, selectedUser: selectedUser, currentPage: currentPage });
      }
      catch (err) {
        Swal.fire("warning!", err.message, "warning");
      } finally {
  ...
  const BindCommissionLedger = async ({ dateRange, selectedUser, currentPage }) => {
    setLoadingTable(true);
    try {
      const _result = await GetCommissionLedger({
        startDate: dateRange?.[0]?.format("YYYY-MM-DD"), endDate: dateRange?.[1]?.format("YYYY-MM-DD"),
        userId: selectedUser, pageNo: currentPage, pageSize: rowsPerPage
      });
      if (_result && Array.isArray(_result.data) && _result.data.length > 0) {        
        setCommissionLedger(_result);
  ...
  const BindDataUserChange = (e) => {
    ...
    BindCommissionLedger({ dateRange: dateRange, selectedUser: userId, currentPage: currentPage });
  }
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Commission Ledger</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            >
              ...
              {UserList && Array.isArray(UserList.data) && UserList.data.length > 0 ?
                UserList.data.filter(x=>x.RoleId===6||x.RoleId===3).map((item) => (
            ...
            <button
              onClick={() => BindCommissionLedger({ dateRange, selectedUser, currentPage })}
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'Open', 'Amount', 'Commission', 'TDS', 'Payable', 'Closed', 'Transaction Type', 'Unique Id', 'Comment', 'Date & Time'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  ) : CommissionLedger && Array.isArray(CommissionLedger.data) && CommissionLedger.data.length > 0 ? (
                    CommissionLedger.data.map((row, index) => (
                      <tr key={index + 1}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white" style={{color: row.TransactionTypeName.toLowerCase()=="credit"?"green":"red"}}>{row.TransactionTypeName}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.SystemUniqueId}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white max-w-[140px] truncate" title={row.Remarks}>{row.Comment}</td>
