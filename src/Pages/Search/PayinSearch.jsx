...
import { SearchPayin, UpdatePendingPayin } from '../../services/Search';
...
import settledwallet from '../../assests/icon/settledWallet.svg';
import unsettledwallet from '../../assests/icon/unsettledWallet.svg';
import checkStatus from '../../assests/icon/checkstatus-icon.png';
import { GetPayinInvoiceLink, PayinCheckStatusTransaction } from '../../services/PayinReport';
...
const PayinSearch = () => {
  ...
  const [rowPayinData, setRowPayinData] = useState(null);
  const [PayinId, setPayinId] = useState(0);
  ...
  const GetSearchPayin = async () => {
    setLoadingTable(true);
    try {
      const _result = await SearchPayin({ searchValue: searchValue, type: searchType });
  ...
  const DownloadPayinInvoice = async (InvoiceId) => {
    try {
  ...
  const UpdatePendingPayinStatus = async (data) => {
    setIsUpdateStatusModalOpen(false);
    try {
      const _result = await UpdatePendingPayin({
        payinId: PayinId, status: data.statusId,
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Search Payin Report</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              onClick={() => GetSearchPayin({ searchValue, searchType })}
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'User Mobile No', 'Service Name', 'Date & Time', 'Status', 'Amount', 'RRN', 'Name', 'Account No', 'IFSC', 'Mobile No', 'UPI Id', 'Open', 'Surcharge', 'GST', 'Payable', 'Closed', 'Ref Id', 'Unique Id', 'Receipt', 'Complaint', 'Chargeback', 'WalletType', 'Fraud', 'Invoice', 'Check Status', 'Check Status Response', 'API Request Response', 'CallBack Response', 'Push CallBack', 'Update Status', 'IP Address'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {loadingTable ? (
                    ...
                    PayinReportList.data.map((row, index) => (
                      <tr key={index + 1} className={`${row.IsFraud.toLowerCase() === "yes" ? "bg-red-500 text-red-700" : "bg-white dark:bg-gray-800"}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <a href='#' onClick={() => { setRowPayinData(row); setIsReceiptModalOpen(true) }} ><BsReceipt size={20} color='black' /></a>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                          {{
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                          {{
                            yes: <img src={success} />,
                            no: <img src={failed} />
                          }[row.RasieChargeBack.toLowerCase()] || (
                              <img src={success} />
                            )}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                          {{
                            settled: <img src={settledwallet} />,
                            unsettled: <img src={unsettledwallet} />
                          }[row.IsInstantPayin.toLowerCase()]}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                          {row.IsFraud.toLowerCase() === "yes" ? <BsEye size={20} onClick={() => { setRowPayinData(row); setIsFraudModalOpen(true) }} /> : "No"}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <a href='#' color='black' onClick={() => DownloadPayinInvoice(row.SystemUniqueId)}>
                            <BsDownload size={20} />
                          </a>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          {row.CheckStatusResponse === "N/A" ? "No Response" : <a href='#' onClick={() => { setRowPayinData(row); setIsCheckStatusModalOpen(true) }}>
                            <BsEye size={20} color='black' />
                          </a>}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <a href='#' onClick={() => { setRowPayinData(row); setIsAPIRequestResponseModalOpen(true) }}>
                            <BsEye size={20} color='black' />
                          </a>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          {row.CallBackResponse === "N/A" ? "No Response" : <a href='#' onClick={() => { setRowPayinData(row); setIsCallBackModalOpen(true) }}>
                            <BsEye size={20} color='black' />
                          </a>}
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{
                          <a href='#' onClick={() => { setPayinId(row.Id); setIsUpdateStatusModalOpen(true) }}> <BsPencil size={20} />
                          </a>
                        }</td>
      ...
      <Modal isOpen={isReceiptModalOpen} onClose={() => setIsReceiptModalOpen(false)}>
        ...
        {[
          { label: "Status", value: rowPayinData?.STATUS },
          { label: "Payment Date", value: rowPayinData?.CreatedDate },
          { label: "Amount", value: new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(rowPayinData?.TransactionAmount) },
          { label: "UTR", value: rowPayinData?.BankPayoutId },
          { label: "Name", value: rowPayinData?.AccountHolderName },
          { label: "Account No", value: rowPayinData?.AccountNo },
          { label: "IFSC", value: rowPayinData?.IfscCode },
          { label: "Payment Invoice no", value: rowPayinData?.SystemUniqueId },
          { label: "Reference Id", value: rowPayinData?.ReferenceId },
        ].map((item, index) => (
      ...
      <Modal isOpen={isCallBackModalOpen} onClose={() => setIsCallBackModalOpen(false)}>
        ...
        <div key={1} className="flex justify-between p-4">
          ...
          <span className="text-gray-800">{rowPayinData?.CallBackDateTime}</span>
        </div>
        <div key={2} className="flex justify-between p-4">
          <span className="text-gray-800" style={{ lineBreak: "anywhere" }}>{rowPayinData?.Response ? rowPayinData?.Response : "No Response Availabel"}</span>
        </div>
      </Modal>
      {/* CallBack end */}
      ...
      <Modal isOpen={isCheckStatusModalOpen} onClose={() => setIsCheckStatusModalOpen(false)}>
        ...
        <div key={1} className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">Date & Time</span>
          <span className="text-gray-800">{rowPayinData?.CheckStatusDateTime}</span>
        </div>
        <div key={2} className="flex justify-between p-4">
          <span className="text-gray-800" style={{ lineBreak: "anywhere" }}>{rowPayinData?.CheckStatusResponse ? rowPayinData?.CheckStatusResponse : "No Response Availabel"}</span>
        </div>
      </Modal>
      {/* CheckStatus end */}
      ...
      <Modal isOpen={IsAPIRequestResponseModalOpen} onClose={() => setIsAPIRequestResponseModalOpen(false)}>
        ...
        <div key={1} className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">Date & Time</span>
          <span className="text-gray-800">{rowPayinData?.CreatedDate}</span>
        </div>
        <div key={2} className="flex justify-between p-4">
          <span className="text-gray-800" style={{ lineBreak: "anywhere" }}>{rowPayinData?.Request ? rowPayinData?.Request : "No Request Availabel"}</span>
        </div>
        <div key={2} className="flex justify-between p-4">
          <span className="text-gray-800" style={{ lineBreak: "anywhere" }}>{rowPayinData?.Response ? rowPayinData?.Response : "No Response Availabel"}</span>
        </div>
      ...
      {/* Fraud start */}
      <Modal isOpen={isFraudModalOpen} onClose={() => setIsFraudModalOpen(false)}>
        <h2 className="text-2xl font-semibold mb-6 border-b border-gray-200 dark:border-gray-700 pb-2">Fraud Details</h2>
        <div className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">Name</span>
          <span className="text-gray-800">{rowPayinData?.FraudName}</span>
        </div>
        <div className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">Account No</span>
          <span className="text-gray-800">{rowPayinData?.FraudAccountNo}</span>
        </div>
        <div className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">UPI Id</span>
          <span className="text-gray-800">{rowPayinData?.FraudUPIId}</span>
        </div>
        <div className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">IP Address</span>
          <span className="text-gray-800">{rowPayinData?.FraudIpv4Address}</span>
        </div>
        <div className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">Device Name</span>
          <span className="text-gray-800">{rowPayinData?.FraudDeviceName}</span>
        </div>
        <div className="flex justify-between p-4">
          <span className="text-gray-600 font-medium">Reason</span>
          <span className="text-gray-800">{rowPayinData?.FraudReason}</span>
        </div>
      </Modal>
      {/* Fraud end */}
      ...
      <Modal isOpen={IsUpdateStatus} onClose={() => setIsUpdateStatusModalOpen(false)}>
        <h2 className="text-2xl font-semibold mb-6 border-b border-gray-200 dark:border-gray-700 pb-2">Update Payin Transaction Status</h2>
