...
import { BindLoanRequest, UpdateLoanRequest } from '../../services/LoanRequest';
...
const validationScheme = Yup.object().shape({
  statusId: Yup.number().typeError("Status is required").moreThan(0, "Please select a valid status").required("Status is required"),
...
const LoanRequestReport = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        BindPendingLoanRequestReport({
          userId: 0, roleId: 0,
          parentUserId: 101, startDate: null, endDate: null, pageNo: 0,
  ...
  const BindDataUserChange = (e) => {
    ...
    GetLoanSummary(null, userId, 0);
  }
  const BindPendingLoanRequestReport = async (data) => {
    try {
      const _result = await BindLoanRequest({
        userId: data.userId, roleId: 0,
        parentUserId: 101, startDate: null, endDate: null, pageNo: 0,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const _result = await UpdateLoanRequest({
        userId: payload.UserId, roleId: payload.RoleId, loanId: payload.Id,
        uplineUserId: payload.ParentUserId, uplineRoleId: payload.ParentRoleId, requestAmount: payload.Amount,
        adminComments: payload.uplinecomment, status: payload.statusid, systemTxnId: payload.SystemUniqueId
      });
       if (isModalOpen) {
      ...
      if (_result.statuscode === 200) {
        ...
        resetFormAndState();
        BindPendingLoanRequestReport({ userId: 0, roleId: 0, parentUserId: 101, startDate: null, endDate: null, pageNo: 0, pageSize: 100, status: StatusEnum.Pending, search: "" });
      } else {        
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Pending Loan Request</h1>
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Mobile No', 'Company name', 'Amount', 'Comments', 'System Txn Id', 'Request Date & Time', 'Status', 'Action'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  ) : PendingloanList && Array.isArray(PendingloanList.data) && PendingloanList.data.length > 0 ? (
                    PendingloanList.data.map((row, index) => (
                      <tr key={index + 1}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                          <button onClick={() => { setSelectloanRow(row); setIsModalOpen(true); }}>Edit</button>
                        </td>
                      </tr>
                    ))
                  ) : (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <h2 className="text-2xl font-semibold mb-6 border-b border-gray-200 dark:border-gray-700 pb-2">Pending Loan Request</h2>
