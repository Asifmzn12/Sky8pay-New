...
const LoanSummaryReport = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        GetLoanSummary({
          dateRange: null, userId: 0, statusId: 0
        });
      }
      catch (err) {
        Swal.fire("warning!", err.message, "warning");
  ...
  const BindDataStatusChange = (e) => {
    ...
    GetLoanSummary(null, 0, statusId);
  }
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Loan Summary</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              type="submit"
              className="px-6 py-2 bg-blue-600 text-white rounded-md shadow hover:bg-blue-700 transition-colors" onClick={() => GetLoanSummary(dateRange, selectedUser, selectedStatus)} >
              Search
            </button>
          </div>
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Mobile No', 'Company name', 'Amount', 'Comments', 'Upline Comments', 'System Txn Id', 'Status', 'Request Date & Time', 'Update Date & Time'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {loadingTable ? (
                    ...
                    LoanSummaryList.data.map((row, index) => (
                      <tr key={index + 1}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.AdminComments}</td>
