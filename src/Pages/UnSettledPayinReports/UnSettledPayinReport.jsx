...
import { GetPayinInvoiceLink, GetUnsettledPayinReport } from "../../services/PayinReport";
  ...
  const BindPayinReport = async ({ dateRange: dateRange, selectedUser: selectedUser, selectedApi: selectedApi, searchValue: searchValue, currentPage: currentPage }) => {
    try {
      const _result = await GetUnsettledPayinReport({
        ...
        apiId: selectedApi, status: StatusEnum.Success, search: searchValue, transactionTypeId: 0, pageNo: currentPage, pageSize: rowsPerPage
      });
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Unsettled Payin Report</h1>
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'Service Name', 'Date & Time', 'Status', 'Amount', 'RRN', 'Name', 'Email Id', 'Payer Mobile No', 'UPI', 'Open', 'Surcharge', 'GST', 'Payable', 'Closed', 'Ref Id', 'Unique Id', 'API Ref Id', 'Receipt', 'Complaint', 'Chargeback', 'Txn From', 'CallBack', 'Check Status', 'Fraud', 'IP Address', 'Invoice', 'IP State', 'IP City', 'Sent Invoice'].map(header => (
                  ...
                  {loadingTable ? (
                    ...
                    PayinReportList.data.map((row, index) => (
                      <tr key={row.Id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.IsInvoiceSent}</td>
...
export default UnSettledPayinReport