...
import { GetUserWiseTds, SaveUserWiseTds } from '../../services/TDS';
...
function UserWiseTDS() {
  ...
  const [CollectedTds, setCollectedTds] = useState(0);
  const [PaidTds, setPaidTds] = useState(0);
  const [RefundTds, setRefundTds] = useState(0);
  const [PayableTds, setPayableTds] = useState(0);
  ...
  const onSubmit = async (data, e) => {
    try {
      ...
      var requestdata = encryptvalue(JSON.stringify({
        ...
        userId: data.userId,
        userPanNo: data.PanNo,
        companyPanNo: data.CompanyPanNo
      }));
      const _result = await SaveUserWiseTds({ data: requestdata });
  ...
  const BindUserWiseTds = async (userId) => {
    try {
      ...
      var _result = await GetUserWiseTds({ data: requestdata });
      setUserTdsList(_result);
      if (_result.statuscode === 200) {
        var _colledtedTds = _result.data.reduce((x, item) => x + item.CollectedTds, 0);
        setCollectedTds(_colledtedTds);
        var _paidTds = _result.data.reduce((x, item) => x + item.PaidUserTds, 0);
        setPaidTds(_paidTds);
        var _refundTds = _result.data.reduce((x, item) => x + item.RefundUserTds, 0);
        setRefundTds(_refundTds);
        var _payableTds = _result.data.reduce((x, item) => x + item.PayableByUser, 0);
        setPayableTds(_payableTds);
      } else {
        setCollectedTds(0);
        setPaidTds(0);
        setRefundTds(0);
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-3xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Use Wise Tds</h1>
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Input Fields */}
            {['Pan No', 'Company Name', 'Company Pan No', 'Amount', 'Comment'].map(field => (
              <div key={field}>
        ...
        {/* Table Section */}
        <div>
          <h2 className="text-2xl font-semibold mb-6 border-b border-gray-200 dark:border-gray-700 pb-2">User Wise TDS</h2>
          <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                ) : UserTdsList && Array.isArray(UserTdsList.data) && UserTdsList.data.length > 0 ? (
                  UserTdsList.data.map((row, index) => (
                    <tr key={index + 1}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{index + 1}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{row.Name}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.CollectedTds)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.PaidUserTds)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.RefundUserTds)}</td>
              ...
              <tfoot>
                <tr>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(CollectedTds)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(PaidTds)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(RefundTds)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(PayableTds)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
