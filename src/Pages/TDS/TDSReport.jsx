...
import { GetTDSReport } from '../../services/TDS';
...
function TDSReport() {
  ...
  const BindTdsReport = async ({ apiId }) => {
    try {
      var requestData = encryptvalue(JSON.stringify({
        apiId: apiId,
        serviceId: 0
      }));
      var _result = await GetTDSReport({ data: requestData });
      setTdsReport(_result);
    } catch (err) {
      Swal.fire("warning!", err.message, "warning");
    }
    finally {
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">TDS Report</h1>
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'Collected TDS', 'Paid', 'Payable'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  ) : TdsReport && Array.isArray(TdsReport.data) && TdsReport.data.length > 0 ? (
                    TdsReport.data.map((row, index) => (
                      <tr key={index + 1}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{index + 1}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.TdsCollection)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.PaidTds)}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR" }).format(row.Payable)}</td>
                      </tr>
                    ))
                  ) : (
                    <tr key={0}>
