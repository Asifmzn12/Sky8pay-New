...
import { GetTDSLedger } from '../../services/TDS';
...
function TDSLedger() {
  ...
  const [TDSLedgerList, setTDSLedgerList] = useState();
  ...
  useEffect(() => {
      (async () => {
        BindTdsLedger({ dateRange: dateRange, apiId: 0 });
      })();
    }, []);
    ...
    const BindTdsLedger = async ({ dateRange, apiId }) => {
      try {
        var requestdata = encryptvalue(JSON.stringify({
        ...
        var _result = await GetTDSLedger({ data: requestdata });
        setTDSLedgerList(_result);
      } catch (err) {
        Swal.fire("warning!", err.message, "warning");
      } finally {
    ...
    const handleApiChange = async (e) => {
      ...
      BindTdsLedger({ dateRange: dateRange, apiId: apiId });
    }
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">TDS Ledger</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              ...
              onClick={() => BindTdsLedger({ dateRange: dateRange, apiId: selectedApi })}
            >
              Search
            </button>
          </div>
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'Pay By', 'Company Name', 'Gst No', 'API Name', 'Unique Id', 'Amount', 'Type', 'Comment', 'Date & Time'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  ) : TDSLedgerList && Array.isArray(TDSLedgerList.data) && TDSLedgerList.data.length > 0 ? (
                    TDSLedgerList.data.map((row, index) => (
                      <tr key={index + 1}>
