import { yupResolver } from '@hookform/resolvers/yup';
import React, { useEffect, useState } from 'react';
import { useForm } from 'react-hook-form';
import * as Yup from 'yup';
import { encryptvalue } from '../../utils/AESEncrypted';
import { BindAPI } from '../../services/Commonapi';
import Swal from 'sweetalert2';
import { SavePayTDS } from '../../services/TDS';

const ValidationError = Yup.object().shape({
  paidBy: Yup.number().typeError("Paid By is required").moreThan(0, "Please select paid by").required("Paid By is required"),
  apiId: Yup.number().typeError("API is required").moreThan(0, "Please select API").required("API is required"),
  GSTNo: Yup.string().required("GST No required"),
  CompanyName: Yup.string().required("Company Name is required"),
  Amount: Yup.number().typeError("Amount is required").moreThan(0, "Please Amount enter greater than 0 ").required("Amount is required"),
  Comment: Yup.string().required("Comment is required")
});
function PayTDS() {
  const [loadingForm, setLoadingForm] = useState(true);
  const [apiOptions, setApiOptions] = useState([]);

  const {
    register,
    handleSubmit,
    reset,
    formState: { errors }
  } = useForm({
    resolver: yupResolver(ValidationError)
  });

  useEffect(() => {
    (async () => {
      apiFetchData();
    })();
  }, []);
  const apiFetchData = async () => {
    try {
      var requestdata = encryptvalue(JSON.stringify({
        RoleId: 2
      }));
      const data = await BindAPI({ data: requestdata });
      if (data && Array.isArray(data.data) && data.data.length > 0) {
        setApiOptions(data.data);
      }
    } catch (err) {
      Swal.fire("warning!", err.message, "warning");
    } finally {
      setLoadingForm(false);
    }
  };

  const onSubmit = async (data, e) => {
    try {
      var btn = e.nativeEvent.submitter;
      const btnval = btn.value;
      var requestData = encryptvalue(JSON.stringify({
        payByUser: data.paidBy,
        amount: data.Amount,
        gstNo: data.GSTNo,
        companyName: data.CompanyName,
        apiId: data.apiId,
        isCredit: Boolean(Number(btnval)),
        comment: data.Comment
      }));
      var _result = await SavePayTDS({ data: requestData });
      if (_result.statuscode === 200) {
        Swal.fire("Success!", _result.message, "success");
        resetFormAndState();
      } else {
        Swal.fire("Error!", _result.message, "error");
      }
    } catch (err) {
      Swal.fire("warning!", err.message, "warning");
    } finally {
      setLoadingForm(false);
    }
  }

  const resetFormAndState = () => {
    reset({
      paidBy: 0,
      apiId: 0,
      GSTNo: "",
      CompanyName: "",
      Amount: "",
      Comment: ""
    });
  }

  return (
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        {/* Main Content Card */}
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-3xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Pay TDS</h1>
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-6">

            {/* Paid By Select */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Select Paid By</label>
              <>
                <select
                  {...register("paidBy")}
                  className={`block w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border rounded-md text-gray-900 dark:text-white ${errors.paidBy ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                    } focus:outline-none focus:ring-2 focus:ring-blue-500`}
                >
                  <option value={0}>Select Paid by</option>
                  <option value={1}>Paid By API</option>
                  <option value={2}>Paid By Me</option>
                </select>
                {errors.paidBy && <p className="text-red-500 text-sm mt-1">{errors.paidBy.message}</p>}
              </>
            </div>
            {/* API Select */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Select API</label>
              {loadingForm ? (
                <div className="h-10 bg-gray-200 dark:bg-gray-700 rounded-md animate-pulse"></div>
              ) : (
                <>
                  <select
                    {...register("apiId")}
                    className={`block w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border rounded-md text-gray-900 dark:text-white ${errors.apiId ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                      } focus:outline-none focus:ring-2 focus:ring-blue-500`}
                  >
                    <option value={0}>Select API</option>
                    {apiOptions && apiOptions.length > 0 ?
                      apiOptions.map((item) => (
                        <option key={item.id} value={item.id}>{item.name}</option>
                      )) : (
                        <option disabled>No Data Found</option>
                      )
                    }
                  </select>
                  {errors.apiId && <p className="text-red-500 text-sm mt-1">{errors.apiId.message}</p>}
                </>
              )}
            </div>
            {/* Input Fields */}
            {['GST No', 'Company Name', 'Amount', 'Comment'].map(field => (
              <div key={field}>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{field.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</label>
                {loadingForm ? (
                  <div className="h-10 bg-gray-200 dark:bg-gray-700 rounded-md animate-pulse"></div>
                ) : (
                  <>
                    <input
                      type="text"
                      {...register(field.replace(/\s+/g, ""))}
                      className={`block w-full px-4 py-2 bg-gray-50 dark:bg-gray-700 border rounded-md text-gray-900 dark:text-white ${errors[field.replace(/\s+/g, "")] ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                        } focus:outline-none focus:ring-2 focus:ring-blue-500`}
                    />
                    {errors[field.replace(/\s+/g, "")] && <p className="text-red-500 text-sm mt-1">{errors[field.replace(/\s+/g, "")].message}</p>}
                  </>
                )}
              </div>
            ))}

            {/* Buttons */}
            <div className="md:col-span-2 flex justify-end gap-4 mt-4">
              <button
                type="submit"
                className="px-6 py-2 bg-green-600 dark:bg-gray-700 text-gray-800 dark:text-white rounded-md hover:bg-green-700 dark:hover:bg-green-600 transition-colors"
                value={1}
              >
                Pay
              </button>
              <button
                type="submit"
                className="px-6 py-2 bg-red-600 text-white rounded-md shadow hover:bg-red-700 transition-colors"
                value={0}
              >
                Refund
              </button>
            </div>
          </form>
        </div>
      </div>
    </div>
  )
}

export default PayTDS