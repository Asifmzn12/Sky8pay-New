...
const UnSettledDebitReport=()=> {
   const rowsPerPage = 50;
    ...
    useEffect(() => {
      (async () => {
        try {
          BindUnSettledDebitReport({ dateRange: dateRange, selectedUser: selectedUser, currentPage: currentPage, searchValue: "" });
        }
        catch (err) {
          Swal.fire("warning!", err.message, "warning");
        } finally {
    ...
    const BindUnSettledDebitReport = async ({ dateRange, selectedUser, currentPage, searchValue }) => {
      ...
      try {
        const _result = await GetUnsettledWallet({
          ...
          pageSize: rowsPerPage, transactionTypeId: 0, reportType: 2
        });
    ...
    const BindDataUserChange = (e) => {
      ...
      BindUnSettledDebitReport({ dateRange: dateRange, selectedUser: userId, currentPage: currentPage, searchValue: "" });
    }
  return (
     <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Unsettled Debit Report</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              onClick={() => BindUnSettledDebitReport({ dateRange, selectedUser, currentPage, searchValue })}
