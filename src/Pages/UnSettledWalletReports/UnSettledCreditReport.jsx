...
const UnSettledCreditReport = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        BindUnSettledCreditReport({ dateRange: dateRange, selectedUser: selectedUser, currentPage: currentPage, searchValue: "" });
      }
      catch (err) {
        Swal.fire("warning!", err.message, "warning");
      } finally {
  ...
  const BindUnSettledCreditReport = async ({ dateRange, selectedUser, currentPage, searchValue }) => {
    ...
    try {
      const _result = await GetUnsettledWallet({
        ...
        pageSize: rowsPerPage, transactionTypeId: 0, reportType: 1
      });
  ...
  const BindDataUserChange = (e) => {
    ...
    BindUnSettledCreditReport({ dateRange: dateRange, selectedUser: userId, currentPage: currentPage, searchValue: "" });
  }
  return (
    <div className="py-4 sm:py-6 md:py-8 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 md:p-10">
          <h1 className="text-1xl font-bold mb-8 text-center text-blue-600 dark:text-blue-400">Unsettled Credit Report</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <button
              onClick={() => BindUnSettledCreditReport({ dateRange, selectedUser, currentPage, searchValue })}
            ...
            <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {['#', 'User Name', 'Company Name', 'Mobile No', 'Service Name', 'Date & Time', 'Open', 'Payable Amount', 'Amount', 'Commission', 'TDS', 'Surcharge', 'GST', 'Closed', 'Unique Id', 'Comment'].map(header => (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {loadingTable ? (
                    ...
                    SettledCreditReportList.data.map((row, index) => (
                      <tr key={row.Id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white" style={{ color: row.ServiceName.toLowerCase() == "debit" ? "red" : "green" }}>{row.ServiceName}</td>
...
export default UnSettledCreditReport